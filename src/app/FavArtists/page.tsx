...
import { ARTIST_BANK } from '@/data/artist';

export default function FavArtistsPage() {
  const { t } = useTranslation();
  const { FavArts, SetFavArts } = useUser();
  const [currentIndex, setCurrentIndex] = useState(0);
  const [searchTerm, setSearchTerm] = useState("");

  const filteredArtists = searchTerm
    ? ARTIST_BANK.filter(artist => 
        artist.name.toLowerCase().includes(searchTerm.toLowerCase())
      )
    : ARTIST_BANK;

  useEffect(() => {
    if (currentIndex >= filteredArtists.length) {
      setCurrentIndex(0);
    }
  }, [filteredArtists.length, currentIndex]);

  const handlePrevious = () => {
    setCurrentIndex((prev) => (prev === 0 ? filteredArtists.length - 1 : prev - 1));
  };

  const handleNext = () => {
    setCurrentIndex((prev) => (prev === filteredArtists.length - 1 ? 0 : prev + 1));
  };

  const toggleArtist = (artistName: string) => {
    if (FavArts.includes(artistName)) {
      SetFavArts(FavArts.filter(a => a !== artistName));
    } else {
      SetFavArts([...FavArts, artistName]);
    }
  };

  const isArtistSelected = (artistName: string) => FavArts.includes(artistName);

  const getVisibleArtists = () => {
    if (filteredArtists.length === 0) return [];
    if (filteredArtists.length === 1) return [0];
    if (filteredArtists.length === 2) return [0, 1];
    
    const prev = currentIndex === 0 ? filteredArtists.length - 1 : currentIndex - 1;
    const next = currentIndex === filteredArtists.length - 1 ? 0 : currentIndex + 1;
    return [prev, currentIndex, next];
  };

  const visibleIndices = getVisibleArtists();
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-slate-900 to-purple-900">
        <div className="max-w-6xl mx-auto px-8 py-16 text-center">
          ...
          <h1 className="text-6xl font-extrabold text-white mb-6 bg-gradient-to-r from-white via-lime-200 to-white bg-clip-text text-transparent">
            {t('favArtists.title') || 'Tus Artistas Favoritos'}
          </h1>
          <p className="text-gray-300 text-xl mb-12 max-w-2xl mx-auto">
            {t('favArtists.subtitle') || 'Selecciona los artistas que más te gustan'}
          </p>
          ...
          <div className="relative max-w-md mx-auto mb-12">
            <input
              ...
              placeholder={t('favArtists.searchPlaceholder') || 'Buscar artista...'}
          ...
          <div className="mt-20 p-8 bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 max-w-md mx-auto">
            <p className="text-gray-300 text-lg">No se encontraron artistas</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto px-8 py-12">
        ...
        <div className="text-center mb-16">
          ...
          <div className="flex items-center justify-between max-w-4xl mx-auto gap-8">
            <span className="text-gray-400 text-lg">{t('favArtists.searchPrompt') || '¿Buscas otro artista?'}</span>
        ...
        <div className="relative mb-20">
          <div className="flex items-center justify-center gap-12">
            ...
            <div className="flex items-center justify-center gap-10 min-h-[450px] relative">
              {visibleIndices.map((index, i) => {
                ...
                return (
                  <div
                    key={`artist-${artist.name}-${index}-${i}`}
                  ...
                  >
                    <div className="relative group cursor-pointer">
                      <div className={`w-[350px] h-[350px] rounded-full overflow-hidden shadow-2xl border-4 transition-all duration-300 ${
                        isSelected 
          ...
          <div className="text-center mt-16 space-y-3">
            <h2 className="text-4xl font-bold text-white">
              {filteredArtists[currentIndex]?.name}
            </h2>
          </div>

          <div className="flex justify-center mt-10">
            <button
              onClick={() => toggleArtist(filteredArtists[currentIndex].name)}
              className={`px-10 py-4 rounded-full font-bold text-lg transition-all duration-300 ${
                isArtistSelected(filteredArtists[currentIndex].name)
            ...
            >
              {isArtistSelected(filteredArtists[currentIndex].name) 
                ? t('favArtists.removeButton') || 'Quitar'
                : t('favArtists.addButton') || 'Agregar'
              }
            </button>
          </div>
          ...
          <div className="text-center mt-8 p-4 bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 max-w-xs mx-auto">
            <p className="text-gray-300 text-lg">
              <span className="text-lime-400 font-bold text-2xl">{FavArts.length}</span> {t('favArtists.artistsSelected') || 'artistas seleccionados'}
            </p>
          </div>
          ...
          <div className="flex justify-end mt-12">
            <Link href="/home">
              <button className="px-14 py-4 bg-gradient-to-r from-blue-500 to-blue-600 text-white font-bold text-lg rounded-xl hover:from-blue-400 hover:to-blue-500 hover:scale-105 transition-all duration-300 shadow-xl shadow-blue-500/30 hover:shadow-blue-500/50">
                {t('favArtists.continueButton') || 'Continuar'} →
              </button>
            </Link>
          </div>
        </div>
